...
- (NSArray *)indexPathsForNodeProperties:(NSArray *)properties atInsertionIndexPath:(NSIndexPath *)insertionIndexPath;
{
    ...
    NSMutableDictionary *childIndexPaths = [NSMutableDictionary dictionary];
    NSMutableDictionary *allIndexPaths = [NSMutableDictionary dictionary];
    NSIndexPath  *childIndexPath, *parentIndexPath;
    
    // The method works because the passed properties are generated by the flattened tree methods which always
    // produce an ordered array of nodes or objects guaranteeing that a node will not be assessed before its parent is.
    NSIndexPath *rootParentIndexPath = nil;
    
    for (NSDictionary *dict in properties) {
        NSString *selfURIpath = [[dict valueForKey:@"selfURI"] path];
        NSString *parentURIpath = [[dict valueForKey:@"parentURI"] path];
        
        // Treat first node specially and set indexPath from insertionIndexPath.
        // Initialise rootParentIndexPath so that it does not point to some random bit of memory and fail 
        // the !rootParentIndexPath test below.
        
        if(!parentURIpath) {
            if(!rootParentIndexPath)
                rootParentIndexPath = insertionIndexPath;
            else
                rootParentIndexPath = [rootParentIndexPath indexPathByIncrementingLastIndex];
        
            [allIndexPaths setValue:rootParentIndexPath forKey:selfURIpath];
            [childIndexPaths setValue:[NSNull null] forKey:selfURIpath];
            [insertedIndexPaths addObject:rootParentIndexPath];
        
        } else {
            childIndexPath = [childIndexPaths valueForKey:parentURIpath];
            if([childIndexPath isEqual:[NSNull null]]) {
                parentIndexPath = [allIndexPaths valueForKey:parentURIpath];
                childIndexPath = [parentIndexPath indexPathByAddingIndex:0];
            } else {
                childIndexPath = [childIndexPath indexPathByIncrementingLastIndex];
            }
            
            [allIndexPaths setValue:childIndexPath forKey:selfURIpath];
            [childIndexPaths setValue:childIndexPath forKey:parentURIpath];
            [insertedIndexPaths addObject:childIndexPath];
        }
    }
//    NSLog(@"allIndexPaths are %@", allIndexPaths);
//    NSLog(@"childIndexPaths are %@", childIndexPaths);
//    NSLog(@"insertedIndexPaths are %@", insertedIndexPaths);
