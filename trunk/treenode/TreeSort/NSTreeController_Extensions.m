...
// Returns an array of indexPaths for the nodes whose properties have been passed
- (NSArray *)indexPathsForNodeProperties:(NSArray *)properties atInsertionIndexPath:(NSIndexPath *)insertionIndexPath;
{
    ...
    // produces an ordered array of nodes or objects guaranteeing that a node will not be assessed before its parent is.
    NSIndexPath *orphanIndexPath = nil;
        
    for (NSDictionary *dict in properties) {
        NSURL *selfURI = [dict valueForKey:@"selfURI"];
        [allIndexPaths setObject:[NSNull null] forKey:selfURI];
        [childIndexPaths setObject:[NSNull null] forKey:selfURI];
    }
        
    for (NSDictionary *dict in properties) {
        NSURL *selfURI = [dict valueForKey:@"selfURI"];
        NSURL *parentURI = [dict valueForKey:@"parentURI"];
        
        // Treat first node specially and set indexPath from insertionIndexPath.
        // Initialise orphanIndexPath so that it does not point to some random bit of memory and fail 
        // the !orphanIndexPath test below.
        
        if(![allIndexPaths objectForKey:parentURI]) {
            if(!orphanIndexPath)
                orphanIndexPath = insertionIndexPath;
            else
                orphanIndexPath = [orphanIndexPath indexPathByIncrementingLastIndex];
        
            [allIndexPaths setObject:orphanIndexPath forKey:selfURI];
            [insertedIndexPaths addObject:orphanIndexPath];
        
        } else {
            childIndexPath = [childIndexPaths objectForKey:parentURI];
            
            if ((NSNull *)childIndexPath == [NSNull null]) {
                parentIndexPath = [allIndexPaths objectForKey:parentURI];
                childIndexPath = [parentIndexPath indexPathByAddingIndex:0];
            } else {
                childIndexPath = [childIndexPath indexPathByIncrementingLastIndex];
            }
            
            [allIndexPaths setObject:childIndexPath forKey:selfURI];
            [childIndexPaths setObject:childIndexPath forKey:parentURI];
